...
// getMgmtDynamicClient returns the dynamic client for the management cluster
func getMgmtDynamicClient() (dynamic.Interface, error) {
	mgmtConfig, _, err := client.GetKubeConfig()
	if err != nil {
		return nil, fmt.Errorf("failed to get management cluster config: %v", err)
	}

	dynamicClient, err := dynamic.NewForConfig(mgmtConfig)
	if err != nil {
...
// handleGetMgmtQuotas returns all quota assignments from the management cluster
func handleGetMgmtQuotas(c *gin.Context) {
	ctx := context.Background()

	dynamicClient, err := getMgmtDynamicClient()
	if err != nil {
		klog.ErrorS(err, "Failed to get dynamic client for management cluster")
		common.Fail(c, err)
		return
	}
	...
	if err != nil {
		klog.ErrorS(err, "Failed to list queues in management cluster")
		common.Fail(c, err)
		return
	}

	// Get profile to user info mapping
	profileUserMap := getProfileUserMapForMgmt(ctx)
...
// handleGetMgmtQuota returns the quota for a specific profile from the management cluster
func handleGetMgmtQuota(c *gin.Context) {
	ctx := context.Background()
	...
	if err != nil {
		if strings.Contains(err.Error(), "not found") {
			common.Fail(c, fmt.Errorf("quota not found for profile: %s in management cluster", profile))
			return
		}
...
// handleCreateMgmtQuota creates a new quota for a profile in the management cluster
func handleCreateMgmtQuota(c *gin.Context) {
	ctx := context.Background()
	...
	if err == nil {
		common.Fail(c, fmt.Errorf("quota already exists for profile: %s in management cluster", req.Profile))
		return
	}
	...
	if err != nil {
		klog.ErrorS(err, "Failed to create queue in management cluster", "profile", req.Profile)
		common.Fail(c, err)
		return
	}

	klog.InfoS("Queue created in management cluster", "profile", req.Profile, "queueName", queueName)
...
// handleUpdateMgmtQuota updates the quota for a specific profile in the management cluster
func handleUpdateMgmtQuota(c *gin.Context) {
	ctx := context.Background()
	profile := c.Param("profile")
	...
	if err != nil {
		klog.ErrorS(err, "Failed to update queue in management cluster", "profile", profile)
		common.Fail(c, err)
		return
	}

	klog.InfoS("Queue updated in management cluster", "profile", profile, "queueName", queueName)
...
// handleDeleteMgmtQuota deletes the quota for a specific profile in the management cluster
func handleDeleteMgmtQuota(c *gin.Context) {
	ctx := context.Background()
	profile := c.Param("profile")
	...
	if err != nil {
		klog.ErrorS(err, "Failed to delete queue in management cluster", "profile", profile)
		common.Fail(c, err)
		return
	}

	klog.InfoS("Queue deleted in management cluster", "profile", profile, "queueName", queueName)
	common.Success(c, gin.H{"message": "Quota deleted successfully"})
}

// getProfileUserMapForMgmt fetches Kubeflow Profile CRs from the management cluster
func getProfileUserMapForMgmt(ctx context.Context) map[string]quotaroutes.ProfileUserInfo {
	profileUserMap := make(map[string]quotaroutes.ProfileUserInfo)

	dynamicClient, err := getMgmtDynamicClient()
	if err != nil {
		klog.V(4).InfoS("Failed to get dynamic client for management cluster", "error", err)
	...
	if err != nil {
		klog.V(4).InfoS("Failed to list Kubeflow Profiles in management cluster", "error", err)
		return profileUserMap
	}
...
func init() {
	r := router.Mgmt()
	r.GET("/quota", handleGetMgmtQuotas)
	r.GET("/quota/:profile", handleGetMgmtQuota)
	r.POST("/quota", handleCreateMgmtQuota)
	r.PUT("/quota/:profile", handleUpdateMgmtQuota)
	r.DELETE("/quota/:profile", handleDeleteMgmtQuota)
	klog.InfoS("Registered management cluster quota routes")
}
