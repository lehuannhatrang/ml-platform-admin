...
// AggregatedQuotaAssignment represents a quota assignment with cluster info
type AggregatedQuotaAssignment struct {
	quotaroutes.QuotaAssignment
	ClusterName string `json:"clusterName"`
}

// AggregatedQuotaListResponse represents the list of all aggregated quota assignments
type AggregatedQuotaListResponse struct {
	Assignments []AggregatedQuotaAssignment `json:"assignments"`
	Total       int                         `json:"total"`
}

// handleGetAggregatedQuotas returns all quota assignments from all member clusters
func handleGetAggregatedQuotas(c *gin.Context) {
	...
	var (
		allAssignments []AggregatedQuotaAssignment
		mu             sync.Mutex
		wg             sync.WaitGroup
	)

	// Fetch quotas from each cluster in parallel
	for _, clusterInfo := range clusters.Clusters {
		// Skip clusters that are not ready
		if clusterInfo.Ready != metav1.ConditionTrue {
			continue
		}
		...
		go func(clusterName string) {
			...
			if err != nil {
				klog.V(4).InfoS("Failed to get dynamic client for cluster", "cluster", clusterName, "error", err)
				return
			}
			...
			if err != nil {
				klog.V(4).InfoS("Failed to list queues in cluster", "cluster", clusterName, "error", err)
				return
			}

			// Get profile to user info mapping for this cluster
			profileUserMap := getProfileUserMapForCluster(ctx, clusterName)

			clusterAssignments := make([]AggregatedQuotaAssignment, 0, len(queues.Items))
			for _, queue := range queues.Items {
				...
				assignment := AggregatedQuotaAssignment{
					QuotaAssignment: quotaroutes.QuotaAssignment{
						Profile:     profile,
						QueueName:   queueName,
				...
				clusterAssignments = append(clusterAssignments, assignment)
			}

			mu.Lock()
			allAssignments = append(allAssignments, clusterAssignments...)
			mu.Unlock()
		}(clusterInfo.ObjectMeta.Name)
	}

	wg.Wait()

	response := AggregatedQuotaListResponse{
...
// handleGetAggregatedQuota returns the quota for a specific profile across all clusters
func handleGetAggregatedQuota(c *gin.Context) {
	ctx := context.Background()
	...
	var (
		assignments []AggregatedQuotaAssignment
		mu          sync.Mutex
		wg          sync.WaitGroup
	)
	...
	// Fetch quota from each cluster in parallel
	for _, clusterInfo := range clusters.Clusters {
		// Skip clusters that are not ready
		if clusterInfo.Ready != metav1.ConditionTrue {
			continue
		...
		go func(clusterName string) {
			...
			// Get profile to user info mapping for this cluster first
			profileUserMap := getProfileUserMapForCluster(ctx, clusterName)
			
			// Skip if the profile doesn't exist in this cluster
			userInfo, exists := profileUserMap[profile]
			if !exists {
				return
			}
			...
			if err != nil {
				// Queue doesn't exist in this cluster, skip
				return
			}
	...
	if len(assignments) == 0 {
		common.Fail(c, fmt.Errorf("quota not found for profile: %s in any cluster", profile))
		return
	}
...
// getProfileUserMapForCluster fetches Kubeflow Profile CRs from a specific cluster
func getProfileUserMapForCluster(ctx context.Context, clusterName string) map[string]quotaroutes.ProfileUserInfo {
	profileUserMap := make(map[string]quotaroutes.ProfileUserInfo)

	// Get the dynamic client for the member cluster
	...
	if err != nil {
		klog.V(4).InfoS("Failed to list Kubeflow Profiles in cluster", "cluster", clusterName, "error", err)
		return profileUserMap
	}
...
func init() {
	r := router.V1()
	r.GET("/aggregated/quota", handleGetAggregatedQuotas)
	r.GET("/aggregated/quota/:profile", handleGetAggregatedQuota)
}
