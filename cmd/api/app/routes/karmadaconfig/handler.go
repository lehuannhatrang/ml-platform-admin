...
func init() {
	r := router.V1()
	r.GET("/karmada-config/running-apps", handleGetRunningapps)
	r.GET("/karmada-config/apps/:appLabel", handleGetMetaData)
	r.GET("/karmada-config/status/:podName", handleCheckDeploymentStatus)
	r.GET("/karmada-config/restart/:podName", handleRestartDeployment)
	r.DELETE("/karmada-config/reinstall/:podName", handleReinstallPod) 
	r.GET("/karmada-config/log/:podName", handleGetPodLogs)
	r.GET("/karmada-config/podinfo/:Name", handlegetpodinfo)
	r.PUT("/karmada-config/podinfo/:Name", hadlleputpodinfo)
}