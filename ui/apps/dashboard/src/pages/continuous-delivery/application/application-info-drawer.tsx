...
import { Drawer, Descriptions, Tag, Typography, Button, Divider, Space, Alert, message, Tabs } from 'antd';
import { SyncOutlined, BarChartOutlined, ClockCircleOutlined, ApartmentOutlined, AppstoreOutlined } from '@ant-design/icons';
import { ArgoApplication, SyncArgoApplication } from '@/services/argocd';
import { calculateDuration } from '@/utils/time';
import { useNavigate } from 'react-router-dom';
import ApplicationResourceGraph from './components/application-resource-graph';
import ApplicationMetrics from './components/application-metrics';
  ...
  return (
    ...
    >
      ...
      {application.status?.conditions && application.status.conditions.length > 0 && (
        <>
          {application.status.conditions.map((condition: { type: string; status: string; message: string }, index: number) => (
            <Alert
              key={index}
      ...
      <Tabs defaultActiveKey="summary">
        ...
        <TabPane 
          tab={
            <span>
              <ApartmentOutlined /> Resource Graph
            </span>
          } 
          key="graph"
        >
          <ApplicationResourceGraph application={application} />
        </TabPane>

        <TabPane 
          tab={
            <span>
              <BarChartOutlined /> Resources
            </span>
          } 
          key="resources"
        >
          {application.status?.resources && application.status.resources.length > 0 ? (
            ...
            <Alert message="No resources found" type="info" showIcon />
          )}
        </TabPane>
