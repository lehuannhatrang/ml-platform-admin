...
const CustomNode: React.FC<CustomNodeProps> = ({ data, id }) => {
  ...
  const tagStyle: CSSProperties = {
    ...
    borderRadius: '4px',
    marginLeft: '4px',
    backgroundColor: nodeRole === 'Master' ? (isDark ? '#d48806' : '#faad14') : (isDark ? '#0958d9' : '#1890ff'),
    color: 'white',
  };

  // Theme-aware status indicator style
  const statusStyle: CSSProperties = {
    display: 'inline-block',
    height: '8px',
    width: '8px',
    borderRadius: '50%',
    backgroundColor: nodeStatus === 'Ready' 
      ? (isDark ? '#52c41a' : '#52c41a') 
      : (isDark ? '#f5222d' : '#f5222d'),
    marginRight: '4px',
  };
  ...
  return (
    <div style={{
      ...
      border: `1px solid ${isDark ? '#52c41a' : '#52c41a'}`,
    }}>
      ...
      <div style={{
        ...
        {nodeRole && <span style={{
          ...tagStyle,
          backgroundColor: nodeRole === 'Master' 
            ? (isDark ? '#d48806' : '#faad14') 
            : (isDark ? '#0958d9' : '#1890ff'),
        }}>{nodeRole}</span>}
      </div>
    </div>
  );
    ...
    nodesData.items.forEach((node) => {
      ...
      newNodes.push({
        ...
        data: {
          ...
          nodeStatus,
          style: {
            background: '#f6ffed', // Will be overridden in CustomNode
